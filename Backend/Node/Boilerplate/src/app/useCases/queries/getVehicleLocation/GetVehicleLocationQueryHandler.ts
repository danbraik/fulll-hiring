...
export class GetVehicleLocationQueryHandler {
  ...
  async handle(query: GetVehicleLocationQuery): Promise<Result<Location | null, Error>> {
    const fleetResult = await this.fleetRepository.findById(FleetId.create(query.fleetId));
